...
def predict_desc(args, normalize=True):
    ...
    filename_tr = os.path.join(args.data_path, args.dataset_name + "_tr_" + str(args.dataset_index) + ".csv")
    filename_vl = os.path.join(args.data_path, args.dataset_name + "_vl_" + str(args.dataset_index) + ".csv")
    filename_te = os.path.join(args.data_path, args.dataset_name + "_te.csv")
    train = pd.read_csv(filename_tr, index_col=0)
    valid = pd.read_csv(filename_vl, index_col=0)
    test = pd.read_csv(filename_te, index_col=0)
    reactivity_data = pd.concat([train, valid, test])
